...
The following is a non-exhaustive list of the capabilities of the library
...
* Parsing and writing Gaussian "cube" files (`cube_format` module). The extracted
  values and the associated `Mesh` (collection of points in space) are represented
  by a `Field` object, which is central to this library.
* Parsing ESP field information from the files generated by Gaussian and saving
  it to a ``resp``-compatible format (`esp_util` module). This is a convenient
  way of obtaining popular fitting meshes, such as MK and CHelpG.
* Reproducing ESP field at points of a mesh from point charges on atoms
  ...
  true ESP field (`calc_fields` module).
* Creating single-molecule, single-structure ``resp`` program input files
...
1. Say that we want to use the CHelpG fitting points as a sampling mesh of the
   ...
   "NM3H_plus_chelpg.esp". The code to parse the file is as follows::

        from repESP.esp_util import parse_gaussian_esp

        with open("NM3H_plus_chelpg.esp") as f:
...
2. Run a Gaussian calculation with the ``Pop=NPA`` option. The NPA charges
   can then be parsed out of the file as follows::
...
5. To reproduce the ESP from our charges, we have to first create a `Molecule`
   object identical to the one in `molecule` variable but with your charges
   instead of CHelpG::
...
* The library uses atomic units for all physical properties. If you're dealing with
...
* This library attempts to enforce consistent use of values describing physical
  properties. You shouldn't need to use "naked" `float` values to represent physical
  properties and should use lightweight classes inheriting from `float` instead.
  For example, if you mean to specify a bond length of 0.5 a.u., you should use
  ``types.Dist(0.5)`` and if you mean to specify a charge of -0.1 e, you should
  use ``charges.Charge(-0.1)``. This aims to ensure that you never try to subtract
  distance from a charge or try to add values in different units. This area of
  the library may not be bulletproof and some arithmetic operations may not
  result in the correct classes and will fall back onto the `float` type,
  which may cause type checker errors. Note that you will be able to use the
  inheriting types like `Dist` in any context you can use a `float`.
* Interacting with this library requires you to use correct types for input
  arguments. The types are documented and additionally the user should run
  a static type checker on their script beforehand, for example::

      mypy example.py

  Using a static type checker will save the user debugging tricky runtime
  errors which tend to occur later in the program. Even if no errors occur, the
  library may produce erroneous results if the passed in arguments don't follow
