...
def resp_optimize_hydrogens(calc_dir_path, respin2_fn, esp_fn, inp_charges,
                            check_ivary=True):
    """Run RESP to optimize hydrogen atoms only

    All heavy atoms will have their charges frozen at input values. Hydrogen
    equivalencing will be taken from the `.respin2` file. A new directory
    `calc_dir_path` will be created to keep all the intermediate and output
    files. The input charges should be given as a list.
    """
    ...
    if len(inp_charges) != len(molecule):
    ...
    charges._update_molecule_with_charges(molecule, inp_charges, 'resp_inp')
    ...
    respin_molecule, charge, iuniq = read_respin2(respin2_fn,
                                                  ref_molecule=molecule)
    write_modified_respin2(respin_molecule, charge, iuniq, calc_dir_path +
                           "input.respin", check_ivary=check_ivary)

    # Run resp
    os.system("cd {0}; resp -i input.respin -o output.respout -e "
              "corrected.esp -q input.qout -t charges.qout"
              .format(calc_dir_path))
    # Update the molecule with new 'resp' charges
    print("\nNOTE: The .qout file was generated by the program, so you can "
          "ignore the following warning.")
    charges.update_with_charges('resp', calc_dir_path + "charges.qout",
                                molecule)
