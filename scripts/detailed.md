...
```
usage: average.py [-h] [--respin_location DIRECTORY] [--esp_file ESP_FILE]
...
```
usage: cavity.py [-h] [-o OUTPUT] INPUT_CUBE
...
```
usage: compromise_cli.py [-h] [--respin_location DIRECTORY]
                         [--limits LOWER UPPER] [--sampling SAMPLING]
                         [--scale_all] [--save_resp_to SAVE_RESP_TO]
                         [-o OUTPUT]
                         INPUT_CHARGE_TYPE INPUT_CHARGE_FILE ESP_FILE_NAME

Calculate compromise charges on the given ESP fitting points. Note that the
input charges should be averaged beforehand and charges passed as a 'list'
type. Nevertheless, this script also works if raw charges are requested to be
extracted from Gaussian or AIMAll output files.
...
optional arguments:
  ...
  --scale_all           Scale charges on all atoms. Note that when used with
                        ...
                        larger fit deterioration than the default 'compromise'
  ...
  --save_resp_to SAVE_RESP_TO
                        ...
                        None)
  -o OUTPUT, --output OUTPUT
                        output file name (default: compromise_charges.txt)
...
```
usage: dipole.py [-h] [--respin_location DIRECTORY]
...
```
usage: eval_fit.py [-h] INPUT_CHARGE_TYPE INPUT_CHARGE_FILE ESP_FILE_NAME
...
```
usage: field_diff.py [-h] [--relative] [--absolute]
...
```
usage: fit_dependence.py [-h] [--respin_location DIRECTORY]
...
```
usage: fit_points.py [-h] [--limits LOWER UPPER]
                     [--color_range LOWER UPPER | --sym_color_range]
                     [-o OUTPUT] [--slice_dist DIST]
...
```
usage: flexibility.py [-h] [--respin_location DIRECTORY]
...
```
usage: plot_fit_dependence1.py [-h] [-o FILENAME] [--plot_fit STATISTIC]
...
```
usage: run_two-stage_resp.py [-h] [--respin_location DIRECTORY]
